...
def test_youtube_api():
    ...
    try:
        ...
        output_file = (
            output_dir / f"youtube_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
ให้เข้าใจง่าย เหมาะสำหรับผู้เริ่มต้น"""
        ...
        response2 = client.chat.completions.create(
            ...
            messages=[
                ...
                {
                    "role": "user",
                    "content": "เสนอ 3 หัวข้อวิดีโอธรรมะที่น่าสนใจสำหรับคนรุ่นใหม่ (แต่ละหัวข้อไม่เกิน 15 คำ)",
                },
            ],
            max_tokens=150,
            temperature=0.8,
        ...
        output_dir.mkdir(parents=True, exist_ok=True)
        output_file = (
            output_dir / f"openai_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
ตอบแบบกระชับ"""
        ...
        output_file = (
            output_dir
            / f"combined_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
def main():
    ...
    results = {"youtube": False, "openai": False, "combined": False}
    ...
    def status_icon(passed: bool) -> str:
        """Return a checkmark or cross icon for a boolean result."""
    ...
    print(
        f"{status_icon(results['openai'])} OpenAI GPT-4 API: {'ผ่าน' if results['openai'] else 'ไม่ผ่าน'}"
    )
    print(
        f"{status_icon(results['combined'])} Combined Workflow: {'ผ่าน' if results['combined'] else 'ไม่ผ่าน/ข้าม'}"
    )
