...
from .model import NotifySummary, TargetResult, NotifyTarget
...
def run(step: dict, run_dir: Path) -> str:
    ...
    if "WEBHOOK_DISABLED" in reason_codes:
        return _write_summary(run_id, timestamp_utc, "skipped", [], "", reason_codes, artifacts_dir)
    ...
    if reason_codes:
        _write_summary(run_id, timestamp_utc, "skipped", [], "", reason_codes, artifacts_dir)
    ...
    if qgate_path.exists():
        try:
             with open(qgate_path, 'r', encoding='utf-8') as f:
                qg_data = json.load(f)
    ...
    for target in targets:
        ...
        t_result = TargetResult(name=target.name, url_redacted=redacted, result="error", http_status=None)
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=timeout_seconds) as response:
                ...
                if 200 <= response.status < 300:
                    ...
                    if "HTTP_ERROR" not in reason_codes: reason_codes.append("HTTP_ERROR")
            ...
            if isinstance(e.reason, socket.timeout):
                ...
                if "TIMEOUT" not in reason_codes: reason_codes.append("TIMEOUT")
            else:
                t_result.result = "error"
                if "CONNECTION_ERROR" not in reason_codes: reason_codes.append("CONNECTION_ERROR")
    ...
    # 6. Write Summary
    path = _write_summary(run_id, timestamp_utc, summary_status, targets_attempted, message_digest, reason_codes, artifacts_dir)
    
    # 7. Fail Open Check
    if summary_status == "failed" and not fail_open:
        raise RuntimeError(f"Notifications failed and fail_open is false. Reasons: {reason_codes}")
