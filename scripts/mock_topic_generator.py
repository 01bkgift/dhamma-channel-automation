...
def generate_topics(templates_data: dict, count: int = 20) -> list[dict]:
    ...
    for category in categories:
        ...
        while (
            len([t for t in topics if t["category"] == category["name"]]) < cat_count
...
def save_topics(topics: list[dict], output_file: Path):
    ...
    for topic in topics:
        # By category
        cat = topic["category"]
        data["statistics"]["by_category"][cat] = (
            data["statistics"]["by_category"].get(cat, 0) + 1
        )

        # By difficulty
        diff = topic["difficulty"]
        data["statistics"]["by_difficulty"][diff] = (
            data["statistics"]["by_difficulty"].get(diff, 0) + 1
        )

        # By season
        season = topic["season"]
        data["statistics"]["by_season"][season] = (
            data["statistics"]["by_season"].get(season, 0) + 1
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate mock topics for Dhamma videos"
    )
    parser.add_argument(
        "--templates",
        type=Path,
        default=Path("data/topic_templates.yaml"),
    ...
    for i, topic in enumerate(topics[:5], 1):
        ...
        print(
            f"   Category: {topic['category']} | Priority: {topic['priority']}/10 | Difficulty: {topic['difficulty']}"
        )
        print(
            f"   Duration: {topic['estimated_duration']} | Audience: {topic['target_audience']}"
        )
        print()
