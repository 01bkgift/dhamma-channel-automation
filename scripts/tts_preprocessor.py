...
def process_file(
    ...
    if verbose:
        print("\n✨ ผลลัพธ์:")
        print(
            f"   • ขนาดเดิม: {metadata['original_length']:,} chars ({metadata['original_bytes']:,} bytes)"
        )
        print(
            f"   • ขนาดใหม่: {metadata['final_length']:,} chars ({metadata['final_bytes']:,} bytes)"
        )
    ...
    output_path = (
        output_path
        or input_path.parent / f"{input_path.stem}_cleaned{input_path.suffix}"
    )
