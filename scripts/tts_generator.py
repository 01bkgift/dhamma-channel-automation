...
def generate_tts(
    text: str, output_path: Path, voice: str = "alloy", speed: float = 1.0
):
    """
    ...
    if text_length > 4096:
        ...
        try:
            ...
            print(
                "   💡 รวมด้วย Audacity: File > Open > เลือกทั้ง 3 ไฟล์ > Tracks > Mix and Render"
            )
            return False
...
def main():
    ...
    parser.add_argument(
        "--speed", type=float, default=1.0, help="Speech speed 0.25-4.0 (default: 1.0)"
    )
    ...
    success = generate_tts(
        text=script_text, output_path=args.output, voice=args.voice, speed=args.speed
    )
