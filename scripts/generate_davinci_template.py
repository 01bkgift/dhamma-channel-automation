...
def generate_edl(visual_guide: dict, output_path: Path, fps: int = 30):
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        for i, scene in enumerate(scenes, 1):
            ...
            f.write(
                f"{i:03d}  AX       V     C        {tc_start} {tc_end} {tc_start} {tc_end}\n"
            )
...
def generate_markdown_guide(visual_guide: dict, output_path: Path):
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        f.write("- **Format:** 1080p 30fps (or 4K if available)\n\n")
        ...
        for i, scene in enumerate(scenes, 1):
            ...
            if scene_type == "B-roll":
                ...
                f.write(
                    f"3. Trim to match end time: {timestamp.split('-')[1] if '-' in timestamp else 'N/A'}\n"
                )
            else:
                f.write("1. Record/import footage for this scene\n")
                f.write(f"2. Place on timeline at {timestamp.split('-')[0]}\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate DaVinci Resolve timeline templates"
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
    ...
    parser.add_argument(
        "--fps", type=int, default=30, help="Timeline framerate (default: 30)"
    )
