...
def create_recording_script(sections: list, guide: dict, output_dir: Path):
    ...
    with open(simple_script, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"Voice Style: {guide.get('voice_profile', {}).get('tone', 'warm, calm')}\n"
        )
        f.write(
            f"Speaking Rate: {guide.get('voice_profile', {}).get('speaking_rate', '120 wpm')}\n"
        )
        f.write("\n" + "=" * 80 + "\n\n")
        ...
        for i, section in enumerate(sections, 1):
            ...
            f.write(
                f"Time: {section['start']} - {section['end']} ({section['duration_sec']}s)\n"
            )
            f.write(
                f"Words: {section['word_count']} (~{section['suggested_wpm']:.0f} wpm)\n"
            )
            f.write("-" * 80 + "\n\n")
    ...
    with open(detailed_script, "w", encoding="utf-8") as f:
        ...
        for i, section in enumerate(sections, 1):
            f.write(f"═══ SECTION {i}/{len(sections)} ═══\n")
            f.write(
                f"⏱  {section['start']} → {section['end']} ({section['duration_sec']}s)\n"
            )
            f.write(
                f"📝 {section['word_count']} words @ ~{section['suggested_wpm']:.0f} wpm\n"
            )
    ...
    for i, section in enumerate(sections, 1):
        section_file = (
            sections_dir
            / f"section_{i:02d}_{section['start'].replace(':', '')}-{section['end'].replace(':', '')}.txt"
        )
        with open(section_file, "w", encoding="utf-8") as f:
            f.write(f"SECTION {i}/{len(sections)}\n")
            f.write(
                f"Duration: {section['duration_sec']}s | Words: {section['word_count']} | Target: {section['suggested_wpm']:.0f} wpm\n"
            )
            f.write("─" * 60 + "\n\n")
...
def main():
    ...
    # สร้างไฟล์
    simple, detailed, metadata, sections_dir = create_recording_script(
        sections, guide, output_dir
    )
    ...
    print(
        f"   📁 {sections_dir.name}/ - Individual section files ({len(sections)} files)\n"
    )
