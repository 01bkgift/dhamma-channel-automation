...
def calculate_statistics(topics_data: dict, history_data: dict) -> dict:
    ...
    upcoming = []
    for topic in sorted(
        topics_data["topics"], key=lambda x: x["priority"], reverse=True
    ):
...
def generate_html(
    topics_data: dict, history_data: dict, stats: dict, output_file: Path
) -> str:
    """สร้าง HTML"""
...
"""
    ...
    for i, topic in enumerate(topics, 1):
        ...
        status_badge = (
            ...
            if is_completed
            else '<span class="badge badge-pending">ยังไม่ทำ</span>'
        )
        diff_class = (
            "easy"
            if topic["difficulty"] == "ง่าย"
            else "medium"
            if topic["difficulty"] == "กลาง"
            else "hard"
        )
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("reports/mock_topics_report.html"),
    ...
    try:
        ...
        if args.output.exists():
            ...
            while True:
                backup_path = (
                    args.output.parent
                    / f"{args.output.stem}_{backup_num:03d}{args.output.suffix}"
                )
