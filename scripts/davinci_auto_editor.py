    ...
    def import_media(self) -> bool:
        ...
        try:
            ...
            if self.broll_dir.exists():
                ...
                if broll_files:
                    broll_paths = [
                        str(f.absolute()) for f in broll_files[:10]
                    ]  # Limit 10
...
def main():
    ...
    parser.add_argument(
        "--production-dir",
        required=True,
        help="Production directory (e.g., output/production_20251105_183328)",
    )
    parser.add_argument(
        "--output",
        required=True,
        help="Output video file (e.g., video/final_video.mp4)",
    )
