...
ตัวอย่าง:
  ...
  python tts_unified.py --provider google --script script.txt --output voice.mp3 --content-only --clean
    ...
    parser.add_argument("--output", type=Path, help="Output audio file path")
    parser.add_argument("--voice", type=str, help="Voice name (provider-specific)")
    parser.add_argument(
        "--speed", type=float, default=1.0, help="Speaking speed (OpenAI)"
    )
    parser.add_argument(
        "--rate", type=float, default=1.0, help="Speaking rate (Google)"
    )
    parser.add_argument("--pitch", type=float, default=0.0, help="Pitch (Google only)")
    parser.add_argument(
        "--list-voices", action="store_true", help="List available voices for provider"
    )
    parser.add_argument(
        "--clean",
        action="store_true",
    ...
    if args.content_only and args.script:
        ...
        # บันทึกไฟล์ชั่วคราว
        temp_script = (
            args.script.parent / f"{args.script.stem}_content_only{args.script.suffix}"
        )
    ...
    if args.clean and args.script:
        ...
        # บันทึกไฟล์ชั่วคราว
        temp_script = (
            args.script.parent / f"{args.script.stem}_cleaned{args.script.suffix}"
        )
        with open(temp_script, "w", encoding="utf-8") as f:
