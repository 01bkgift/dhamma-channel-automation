...
def assert_json_field(path: Path, field: str, expected_value, description: str):
    ...
    try:
        ...
        if actual_value != expected_value:
            log(f"FAIL: {description} (Field '{field}'): Expected '{expected_value}', Got '{actual_value}'", "ERROR")
            return False
...
def run_test(run_id: str, dry_run: bool = False):
    ...
    # A. Voiceover Summary
    all_passed &= assert_file_exists(artifacts_dir / "voiceover_summary.json", "Voiceover Summary")
    
    # B. Video Render Summary
    all_passed &= assert_file_exists(artifacts_dir / "video_render_summary.json", "Video Render Summary")
    ...
    # D. Quality Gate
    all_passed &= assert_file_exists(artifacts_dir / "quality_gate_summary.json", "Quality Gate Summary")
    all_passed &= assert_json_field(artifacts_dir / "quality_gate_summary.json", "decision", "pass", "Quality Gate Decision")
    
    # E. Decision Support
    all_passed &= assert_file_exists(artifacts_dir / "decision_support_summary.json", "Decision Support Summary")
    
    # F. Approval Gate (Should be approved automatically due to grace_period=0)
    all_passed &= assert_file_exists(artifacts_dir / "approval_gate_summary.json", "Approval Gate Summary")
    all_passed &= assert_json_field(artifacts_dir / "approval_gate_summary.json", "status", "approved_by_timeout", "Approval Status")
...
def main():
    ...
    parser.add_argument("--run-id", help="Custom run ID", default=f"e2e_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
    parser.add_argument("--dry-run", action="store_true", help="Run orchestrator in dry-run mode (no actual generation)")
