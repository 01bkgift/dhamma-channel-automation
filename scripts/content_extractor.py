...
class ContentExtractor:
    ...
    def extract_content(self, text: str) -> tuple[str, dict]:
        ...
        content_lines = []
        removed_count = {"metadata": 0, "directions": 0, "instructions": 0, "empty": 0}
    ...
    def analyze(self, text: str) -> dict:
        ...
        # นับจำนวน
        analysis["metadata_lines"] = len(
            re.findall(r"^SECTION\s+\d+/\d+", text, re.MULTILINE)
        )
...
def process_file(
    input_path: Path, output_path: Path = None, verbose: bool = True
) -> dict:
    """
    ประมวลผลไฟล์สคริปต์
    """
    ...
    if verbose:
        ...
        print(
            f"   • ขนาดเนื้อหา: {metadata['content_length']:,} chars ({metadata['content_bytes']:,} bytes)"
        )
    ...
    output_path = (
        output_path
        or input_path.parent / f"{input_path.stem}_content_only{input_path.suffix}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input", "-i", type=str, required=True, help="ไฟล์ input")
    parser.add_argument(
        "--output",
        "-o",
        type=str,
    ...
    parser.add_argument("--quiet", "-q", action="store_true", help="ไม่แสดงรายละเอียด")
