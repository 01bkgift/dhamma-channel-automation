...
def run_path_a(config: dict, input_dir: Path):
    ...
    if config.get("download_broll", False):
        ...
        cmd = [
            ...
            str(broll_dir),
            "--max-videos",
            str(config.get("max_broll_videos", 10)),
        ]
        ...
        steps.append(
            {
                "cmd": cmd,
                "desc": "ดาวน์โหลดวิดีโอ B-roll"
                if api_key
                else "แสดงรายการความต้องการ B-roll (โหมดจำลอง)",
            }
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Production Orchestrator - Run production workflow"
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("production_config.json"),
    ...
    if missing_files:
        print("⚠️  คำเตือน: ไฟล์ต่อไปนี้หายไปในโฟลเดอร์อินพุต:")
