...
class FullAutoOrchestrator:
    ...
    def _update_metadata_with_duration(self):
        ...
        try:
            ...
            if duration_sec > 0:
                ...
                metadata["actual_duration_formatted"] = (
                ...
                print(
                    f"   ⏱️  Duration: {metadata['actual_duration_formatted']} ({duration_sec:.1f}s)"
                )
    ...
    def step3_video_editing(self) -> bool:
        ...
        print(
            "💡 Will try DaVinci Resolve first; if unavailable, will use fallback renderer"
        )
        ...
        print(
            "⚠️  DaVinci Resolve not available or failed. Falling back to simple renderer..."
        )
    ...
    def step5_youtube_upload(self) -> bool:
        ...
        if not self.video_file.exists():
            ...
            print(
                f"   python scripts/youtube_uploader.py --production-dir {self.output_dir} --video [your_video.mp4]"
            )
            return True
        ...
        if response.lower() != "y":
            ...
            print(
                f"   python scripts/youtube_uploader.py --production-dir {self.output_dir} --video {self.video_file} --thumbnail {self.thumbnail_file}"
            )
            return True
    ...
    def run(self) -> bool:
        ...
        for _i, (step_func, step_name) in enumerate(steps, 1):
            try:
                success = step_func()
                if not success:
...
def main():
    ...
    parser.add_argument(
        "--topic", help="Video topic (optional - will pick from database)"
    )
