...
def convert_pause_to_ssml(text: str) -> tuple[str, bool]:
    ...
    def replace_pause(match):
        ...
        duration_match = re.search(
            r"\[PAUSE\s+(\d+(?:\.\d+)?)(s|ms)?\]", full_match, re.IGNORECASE
        )
...
def generate_tts_google(
    ...
    try:
        ...
        print("✅ เชื่อมต่อ Google Cloud TTS สำเร็จ")
    ...
    if text_bytes > MAX_BYTES:
        print(
            f"⚠️ ข้อความยาวเกิน {MAX_BYTES} bytes ({text_bytes:,} bytes) จะแบ่งเป็นหลายส่วน..."
        )
        ...
        for line in lines:
            ...
            if line[-1] not in ".!?" and not line.startswith("[PAUSE"):
    ...
    if output_path.exists():
        ...
        # คำนวณค่าใช้จ่าย
        cost_per_million = (
            16 if "Wavenet" in voice_name or "Neural" in voice_name else 4
        )
...
ตัวอย่าง:
  python tts_generator_google.py --script script.txt --output voice.mp3 --voice th-TH-Wavenet-B
    ...
    parser.add_argument(
        "--script", type=Path, help="Path to script file (recording_script_SIMPLE.txt)"
    )
    parser.add_argument(
        "--output", type=Path, help="Output audio file path (voiceover_ai.mp3)"
    )
    parser.add_argument(
        "--voice",
        type=str,
        default="th-TH-Journey-D",
    ...
    parser.add_argument(
        "--pitch", type=float, default=0.0, help="Pitch -20.0 to 20.0 (default: 0.0)"
    )
    parser.add_argument(
        "--list-voices", action="store_true", help="List all available Thai voices"
    )
