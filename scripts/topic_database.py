...
class TopicDatabase:
    ...
    def get_next_topic(
        self, skip_completed: bool = True, min_priority: int = 3
    ) -> dict | None:
        ...
        sorted_topics = sorted(
            self.topics_data["topics"], key=lambda x: x["priority"], reverse=True
        )

        # หาหัวข้อแรกที่ยังไม่ทำและมี priority >= min_priority
        for topic in sorted_topics:
            if (
                topic["id"] not in completed_ids
                and topic.get("priority", 0) >= min_priority
            ):
                return topic
    ...
    def mark_failed(self, topic_id: str, run_id: str, error_message: str):
        ...
        self.history_data["in_progress"] = [
            r for r in self.history_data["in_progress"] if r["topic_id"] != topic_id
        ]
...
def main():
    ...
    parser.add_argument(
        "--topics",
        type=Path,
        default=Path("data/mock_topics.json"),
        help="Path to topics JSON file",
    )
    parser.add_argument(
        "--history",
        type=Path,
        default=Path("data/production_history.json"),
    ...
    parser_next.add_argument(
        "--title-only",
        action="store_true",
        help="Print only the title (for batch scripts)",
    )
    parser_next.add_argument(
        "--id-only", action="store_true", help="Print only the ID (for batch scripts)"
    )
    ...
    parser_upcoming.add_argument(
        "--count", type=int, default=5, help="Number of topics"
    )
    ...
    parser_mark.add_argument(
        "--status", choices=["completed", "in_progress", "failed"], required=True
    )
    ...
    if args.command == "next":
        ...
        for i, topic in enumerate(topics, 1):
            ...
            print(
                f"   ID: {topic['id']} | Priority: {topic['priority']}/10 | {topic['category']}"
            )
            print()
        ...
        if args.status == "completed":
            if not args.run_id or not args.output_dir:
                print(
                    "❌ Error: --run-id and --output-dir required for completed status"
                )
                return 1
