...
from src.steps.notify_webhook.model import NotifySummary
...
def test_no_targets(mock_run_dir):
    with mock.patch.dict(os.environ, {"NOTIFY_ENABLED": "true", "NOTIFY_WEBHOOKS_JSON": "[]"}, clear=True):
        notify_step.run({}, mock_run_dir)
...
def test_redaction_in_summary(mock_run_dir, basic_env, mock_decision_artifact):
    with mock.patch("urllib.request.urlopen") as mock_urlopen:
        ...
        assert target["url_redacted"].endswith("***oken") or target["url_redacted"].endswith("***oken") # 'token' last 4 is 'oken'
...
def test_duplicate_target_names(mock_run_dir, mock_decision_artifact):
    ...
    with mock.patch.dict(os.environ, {
        "NOTIFY_ENABLED": "true",
        "NOTIFY_WEBHOOKS_JSON": json.dumps(targets)
    }, clear=True):
        with mock.patch("urllib.request.urlopen") as mock_urlopen:
             mock_response = mock.Mock()
             mock_response.status = 200
